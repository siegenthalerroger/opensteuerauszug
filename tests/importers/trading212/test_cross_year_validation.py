...
in the private directory, then run::
...
def _discover_csv_files() -> Dict[int, Path]:
    """Return {year: path} for all t212-export-YYYY*.csv files in private directory."""
    result: Dict[int, Path] = {}
    private_dir = _PROJECT_ROOT / "private"
    for p in sorted(private_dir.glob("t212-export-*.csv")):
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        ...
        f"in the private directory ({_PROJECT_ROOT / 'private'}). "
